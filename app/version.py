...
# By default we expect to find a lightweight tag in the history
...
with subprocess.Popen(
    ["/usr/bin/git", "describe", "--tags"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
) as proc:
...
if GIT_VERSION == "":
    ...
    with subprocess.Popen(
        ...
        stdout, stderr = proc.communicate()
    __version__ = f"v{stdout.decode('utf-8').strip()}"
else:
